...
EC2_PRICING_REGION_INDEX_URL = f"{AWS_PRICING_API_ROOT}/offers/v1.0/aws/AmazonEC2/current/region_index.json"
...
def fetch_ondemand_prices(aws_region: str) -> t.Mapping[str, float]:
    ...
    region_url = AWS_PRICING_API_ROOT + region_data["regions"][aws_region]["currentVersionUrl"]
    ...
    skus_by_instance_type = {
        ...
        if deets["attributes"].get("usagetype").startswith("BoxUsage") and
            deets["attributes"]["operatingSystem"] == "Linux"  # noqa
    }
    ...
    for instance_type, sku in skus_by_instance_type.items():
        ...
        prices_by_instance_type[instance_type] = float(list(price_dimensions.values())[0]["pricePerUnit"]["USD"])

    return prices_by_instance_type


def fetch_spot_prices(ec2: EC2Client) -> t.Mapping[t.Tuple[InstanceTypeType, str], float]:
    ...
    for page in paginator.paginate(
        ...
        for item in page["SpotPriceHistory"]:
            spot_prices_by_instance_type[(item["InstanceType"], item["AvailabilityZone"])] = float(item["SpotPrice"])

    return spot_prices_by_instance_type


def make_resource_map(instance_type: InstanceTypeInfoTypeDef) -> t.MutableMapping[str, str]:
    resource_map = {}
...
def is_valid_usage_class(
    ...
    return (
        cl == "on-demand" or
        (cl == "spot" and not disable_spot and (name, zone) in spot_instances)
    )
